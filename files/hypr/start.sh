...
discord &
sleep 3  # Give Discord more time
...
# Make sure Discord is floating
if [[ "$(hyprctl clients -j | jq -r --arg addr "$discord" '.[] | select(.address == $addr) | .floating')" != "true" ]]; then
...
# Position Discord first (far right)
